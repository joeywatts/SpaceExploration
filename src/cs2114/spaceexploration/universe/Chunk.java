 ...
 */
public class Chunk extends BaseObject3D {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int j = 0; j < 2; j++) {
				...
				for (int k = 0; k < 2; k++) {
					...
					temp += temperature[i][j][k] * widthInterp * heightInterp
							* depthInterp;
				}
			}
		}
	...
	public boolean tessellate(int lodLevel) {
		...
		setData(tessellator.getVertices(), tessellator.getNormals(),
				tessellator.getTextureCoords(), tessellator.getColors(),
