...
// -------------------------------------------------------------------------
/**
 * NoisePlanetGenerator is a PlanetGenerator that uses Noise.
 *
 * @author jwatts96
 * @author garnesen
 * @author jam0704
    ...
    implements PlanetGenerator
{
    ...
    /**
     * Instantiates a new NoisePlanetGenerator.
     *
     * @param planetSeed
     *            the planet's seed.
     */
    ...
    public void generateChunk(Chunk c)
    {
        ...
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
            {
                ...
                {
                    noiseVals[x][y][z] =
                        calculateDensity(chunkX + SAMPLE_RATE_X * x, chunkY
                            + SAMPLE_RATE_Y * y, chunkZ + SAMPLE_RATE_Z * z);
                }
            }
        }
        for (int x = 0; x <= Chunk.SIZE; x++)
        {
            ...
            for (int y = 0; y <= Chunk.SIZE; y++)
            {
                ...
                for (int z = 0; z <= Chunk.SIZE; z++)
                {
                    ...
                    float val = 0;
                    val +=
                        noiseVals[x1i][y1i][z1i] * (1 - xd) * (1 - yd)
                            * (1 - zd) + noiseVals[x2i][y1i][z1i] * xd
                            * (1 - yd) * (1 - zd);
                    val +=
                        noiseVals[x1i][y2i][z1i] * (1 - xd) * yd * (1 - zd)
                            + noiseVals[x2i][y2i][z1i] * xd * yd * (1 - zd);
                    val +=
                        noiseVals[x1i][y1i][z2i] * (1 - xd) * (1 - yd) * zd
                            + noiseVals[x2i][y1i][z2i] * xd * (1 - yd) * zd;
                    val +=
                        noiseVals[x1i][y2i][z2i] * (1 - xd) * yd * zd
            ...
            for (int y = 0; y < 2; y++)
            {
                for (int z = 0; z < 2; z++)
                {
                    c.setTemperature(
                        ...
                        y,
                        z,
                        calculateTemperature(chunkX + x * Chunk.SIZE, chunkY
                            + y * Chunk.SIZE, chunkZ + z * Chunk.SIZE));
                }
            }
        }
                ...
                for (int z = 0; z < Chunk.SIZE; z++)
                {
                    chunk
                        .setDensity(
                            ...
                            z,
                            calculateDensity(
                                (int)(x * interval - radius),
                                (int)(y * interval - radius),
                                (int)(z * interval - radius)));
                }
            }
        }
        for (int x = 0; x < 2; x++)
            ...
            for (int y = 0; y < 2; y++)
            {
                for (int z = 0; z < 2; z++)
                {
                    chunk.setTemperature(
                        ...
                        y,
                        z,
                        calculateTemperature(-radius + x * radius * 2, -radius
                            + y * radius * 2, -radius + z * radius * 2));
                }
            }
        }
        chunk.tessellate(1);
    ...
    /**
     * Calculates the density at a certain point.
     *
     * @param x
     *            the x component.
     * @param y
    ...
    /**
     * Calculates the temperature at a certain point.
     *
     * @param x
     *            the x component.
     * @param y
     ...
     * @param z
     *            the z component.
     * @return the temperature at (x, y, z)
     */
